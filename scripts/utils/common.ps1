...
$curr = (Resolve-Path "$PSScriptRoot\..").Path
if (Test-Path (Join-Path $curr "backend")) {
    $script:RepoRoot = $curr
} else {
...
function Write-CheckPass { param([string]$Label) Write-Host ("  [PASS] {0,-20}" -f $Label) -ForegroundColor Green }
function Write-CheckFail { param([string]$Label) Write-Host ("  [FAIL] {0,-20}" -f $Label) -ForegroundColor Red }
function Write-CheckSkip { param([string]$Label) Write-Host ("  [SKIP] {0,-20} (not installed)" -f $Label) -ForegroundColor Yellow }
