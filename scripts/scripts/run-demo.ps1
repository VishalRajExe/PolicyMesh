...
& "$PSScriptDir\run-ci-check.ps1" -Scenario valid -NoColor 2>$null
...
& "$PSScriptDir\run-ci-check.ps1" -Scenario blocked -NoColor 2>$null
if ($LASTEXITCODE -eq 0) {
...
try {
    ...
    if ($backendToken) { $headers["Authorization"] = "Bearer $backendToken" }
    $body = '{"source":{"service":"orders-api","region":"EU"},"target":{"service":"payments-api","region":"EU"},"dataClassification":"PII"}'
    $resp = Invoke-RestMethod -Uri "$backendUrl/api/v1/compliance/check" -Method Post -Headers $headers -Body $body -TimeoutSec 10 -ErrorAction Stop
    ...
    Write-Host ("  ⏭  Runtime EU -> EU: (compliance API not available)") -ForegroundColor Yellow
}
Write-Host ""
...
try {
    $headers = @{ "Content-Type" = "application/json" }
    if ($backendToken) { $headers["Authorization"] = "Bearer $backendToken" }
    $body = '{"source":{"service":"orders-api","region":"EU"},"target":{"service":"analytics-api","region":"US"},"dataClassification":"PII"}'
    $resp = Invoke-RestMethod -Uri "$backendUrl/api/v1/compliance/check" -Method Post -Headers $headers -Body $body -TimeoutSec 10 -ErrorAction Stop
    ...
    Write-Host ("  ⏭  Runtime EU -> US: (compliance API not available)") -ForegroundColor Yellow
}
Write-Host ""
...
try {
    $aiResp = Invoke-WebRequest -Uri "$aiServiceUrl/health" -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop
    Write-CheckPass "AI Service: available"
} catch {
    Write-Host ("  ⏭  AI Service: (not running)") -ForegroundColor Yellow
}
Write-Host ""
