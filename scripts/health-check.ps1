...
$dbPort = if ($env:DB_PORT) { [int]$env:DB_PORT } elseif ($env:MYSQL_PORT) { [int]$env:MYSQL_PORT } else { 3306 }
...
# MySQL
$script:total++
if (Test-PortInUse -Port $dbPort) {
    Write-CheckPass "MySQL"
    $script:passed++
} else {
    Write-Host ("  ⏭  {0,-16} NOT RUNNING" -f "MySQL") -ForegroundColor Yellow
    $script:total--
}
